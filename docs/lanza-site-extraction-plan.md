...
- **P4 — Code DONE (2026-07-04); publish/template are deploy actions (Dave).**
  Code: (a) identity consolidated into repo-root `lanza.config.json`
  `{owner,name,adminLogin}` — deleted `functions/_lib/tenant-owner.ts`, `handoff.ts`
  reads adminLogin from config, broker writes the one file; (b) `lanza build` copies
  the package's `functions/` into the tenant REPO ROOT (Pages compiles functions/ from
  root, not node_modules), safe now that functions/ is pure package code; skipped in
  the dogfood. Verified: external tarball-install tenant copies functions/ to its root,
  copied proxy resolves the TENANT's config (owner=testowner). **Deploy actions left to
  Dave:** `npm publish` `@lanza/site` (flip `private`), create the thin content-only
  template repo, connect it on Pages. **ASSUMPTION to confirm on first external deploy:**
  Pages compiles a `functions/` dir generated by the build command (fallback: commit thin
  functions in the template). Does NOT affect the dogfood (its functions/ is committed at root).
