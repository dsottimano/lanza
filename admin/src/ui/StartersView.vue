...
const preview = computed(() => {
  const starter = STARTERS.find(s => s.id === selected.value)!;
  const style = starter.looks.find(s => s.id === look.value)!;
  const brand = resolveBrand({ brand: style.brand } as Parameters<typeof resolveBrand>[0]);
  return `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;${brand.styleVars};background:var(--paper)}*{box-sizing:border-box}</style></head><body>${render(starter.templates[starter.home.preset].html, starter.home.slots)}</body></html>`;
});
    ...
    <fieldset :disabled="busy">
      ...
      <div class="starter-options">
        <button v-for="starter in STARTERS" :key="starter.id" type="button" class="starter-option" :class="{ chosen: selected === starter.id }" :aria-pressed="selected === starter.id" @click="selected = starter.id">
          <span class="eyebrow">{{ starter.id === 'portfolio' ? 'Selected work · Case studies' : 'Properties · Places' }}</span>
      ...
      <div class="looks">
        <button v-for="style in STARTERS[0].looks" :key="style.id" type="button" :aria-pressed="look === style.id" :class="{ chosen: look === style.id }" @click="look = style.id">
      ...
      <h2>Preview</h2><iframe title="Starter design preview" sandbox="" :srcdoc="preview" style="width:100%;height:480px;border:1px solid #d8d5ca" />
      <div class="setup">
        ...
        <label><input v-model="samples" type="checkbox" /> Include example projects or properties</label>
      </div>
      ...
      <p class="muted">Not included: {{ STARTERS.find(s => s.id === selected)?.notIncluded.join(', ') }}.</p>
