...
- [x] **Phase 5 — Render switch + cutover.**
      ...
      - **5b (cutover, shipped):** Studio builds to `public/admin/` (base `/admin/`,
        ...
        `CLAUDE.md` + `README` updated (Sveltia → Studio, markdown → HTML).

## How to run

- **Studio with HMR:** `npm --prefix cms run dev` → http://localhost:5173/admin/index.html.
- **Full deploy build (incl. Studio):** `npm run build` from repo root →
  builds the CMS into `public/admin`, then `astro build` copies it to `dist/admin`.
- **Build only Studio:** `npm run build:studio` (or `cd cms && npm run build`) → `public/admin/`.
  (`public/admin/` is gitignored — regenerated by the build.)
- **Typecheck Studio:** `cd cms && npm run typecheck`.
...
- **Body format:** posts are `.md` with **HTML bodies**. As of Phase 5, Astro
  ...
  a body that is still raw markdown (e.g. a bot draft never opened in Studio) will
  render unprocessed. The workflow avoids this: publishing goes through Studio,
...
- **Done (Phase 5) — sanitize on the public render.** Bodies render via
  ...
  being opened (and thus sanitized) in Studio.
