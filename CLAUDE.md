...
Static Astro site (repo root) + **Lanza**, a self-owned Vue 3 + TipTap CMS
(`lanza/`, served at `/admin`) + a Telegram draft bot (`bot/`), all on Cloudflare's
...
4. **Lanza is the CMS, and it's ours.** Source in `lanza/` (Vue 3 + Vite + TS +
   TipTap). The content model lives in `lanza/src/schema.ts` (the single source of
   truth — there is no `config.yml`). It builds to `public/admin/` (gitignored,
   regenerated by `npm run build:lanza`) and runs as a static SPA — no Worker.
...
- Path moved? → update `lanza/src/schema.ts` (collection `folder`), `bot/wrangler.jsonc`
  (`CONTENT_DIR`), and `scripts/gen-redirects.mjs` together.
- Changed the CMS (`lanza/`)? → rebuild (`npm run build:lanza`) and confirm `/admin`
