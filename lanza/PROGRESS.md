...
- [x] **Phase 3 — Structured fields + other collections.** Typed schema module
      (`lanza/src/schema.ts`) replaces `config.yml`. Schema-driven, recursive field
      renderer (`lanza/src/fields/`). Posts/pages get a Ghost-style **settings
...
- [x] **Phase 5 — Render switch + cutover.**
      - **5a (shipped):** the 2 Astro detail templates render `entry.body` via
        `set:html`, **sanitized at build** (`src/lib/sanitize.ts`,
...
- **Lanza with HMR:** `npm --prefix lanza run dev` → http://localhost:5173/admin/index.html.
...
- **Build only Lanza:** `npm run build:lanza` (or `cd lanza && npm run build`) → `public/admin/`.
  (`public/admin/` is gitignored — regenerated by the build.)
- **Typecheck Lanza:** `cd lanza && npm run typecheck`.
...
- **Fixed — iframe/img src validation** (`lanza/src/editor/url.ts`). A crafted draft
...
- **Done (Phase 5) — sanitize on the public render.** Bodies render via
  `set:html`, sanitized at build time in `src/lib/sanitize.ts`
...
- `lanza/src/schema.ts` — the content model (collections + fields). **Single source
...
- `lanza/src/editor/` — TipTap editor, card nodes (`extensions/`), node-views
  (`nodeviews/`), slash menu.
- `lanza/src/fields/` — schema-driven form: `FieldForm.vue` (root, provides client),
...
- `lanza/src/backend/` — `github.ts` (generic Contents API: `listDir`/`loadEntry`/
  ...
  `MEDIA` coords).
- `lanza/src/ui/` — `LoginView`, `Sidebar` (collection rail), `CollectionList`
